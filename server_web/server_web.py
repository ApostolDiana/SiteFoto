...
while True:
	...
	elementeLineDeStart = linieDeStart.split()
 	# inregistrare
	if(elementeLineDeStart[1] == '/api/utilizatori'):
		element=cerere.split('\r\n')[-1]
		...
		with open('../continut/resurse/utilizatori.json', 'r+') as f:
			...
			f.truncate()
			clientsocket.sendall('HTTP/1.1 201 OK\r\n'.encode('utf-8'))
	# TODO securizare
	...
	try:
		...
		clientsocket.sendall('Content-Length: '.encode('utf-8') + str(len(msg.encode('utf-8'))).encode('utf-8') + '\r\n'.encode('utf-8'))
